...
def train(config):
    ...
    for epoch in range(config.max_epoch):
        ...
        for idx, noise in enumerate(train_loader):
            ...
            # clean_spectra: 스펙트럼 생성기 (가상의 '클린' 스펙트럼)
            clean_spectra = gen_train.generator(spec, bsz).T  # (spec, bsz)
            # noisy_spectra: 실제 노이즈 + 클린
            ...
            # --- 추가: 노이즈의 '정규화된 DCT 계수'를 한번 시각화 ---
            # epoch=0이고, idx=0일 때 첫 배치만 그려봅니다 (너무 자주 그리면 느려집니다)
            if epoch == 0 and idx == 0:
                plt.figure("Train: Normalized Noise DCT (epoch=0, idx=0)")
                plt.plot(target_norm[0, 0, :], label="Noise's normalized DCT (first sample)")
                plt.title("Normalized Noise DCT Coefficients (first batch sample)")
        ...
        print(f"[Epoch {epoch}] Global Step: {global_step} | "
              f"Train Loss: {epoch_train_loss:.6f} | Valid Loss: {valid_loss:.6f}", flush=True)
...
def batch_predict(config):
    ...
    for i in range(N):
        ...
        # (추가) 첫 번째 스펙트럼의 "Predicted Noise DCT" 그래프를 따로 출력
        if i == 0:
            plt.figure("BatchPredict: Predicted Noise DCT (Sample 0)")
            plt.plot(pred_out, label="Predicted Noise (DCT) for Sample 0")
            plt.xlabel("Coefficient Index")
            plt.ylabel("Amplitude")
            plt.title("Predicted Noise in DCT domain (Sample 0)")
            plt.legend()
            plt.show()
