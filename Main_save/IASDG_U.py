...
def precompute_DTD(L, lam):
    ...
    return lam * D.T.dot(D)           # 결과 shape: (L, L)
...
def als_baseline(y, lam=1e5, p=0.01, niter=10, visualize=False, DTD=None):
    ...
    Args:
        ...
        DTD (2D np.array, optional): 미리 계산된 lam * (D^T D) (크기 (L, L)). 제공되면 이를 사용.
...
def train(config):
    ...
    global_step = -1  # 첫 배치 후 0부터 시작하도록
    ...
    for epoch in range(config.max_epoch):
        ...
        for idx, noise in enumerate(train_loader):
            ...
            noisy_spectra = clean_spectra + noise  # noisy signal = clean + noise
        ...
        model.eval()
        valid_loss = 0.0
        # 마지막 validation 배치의 clean_spectra와 noisy_spectra를 기억함.
        ...
        # 100 에포크마다 sample prediction 플롯 (validation 마지막 배치의 noisy_spectra, clean_spectra 사용)
        if (epoch+1) % 100 == 0:
        ...
        # valid_loss 기준으로 베스트 모델 저장 (상위 3개만 유지)
        if len(best_models) < 3 or valid_loss < max(best_models, key=lambda x: x[0])[0]:
            ...
            print(f"[Save] Checkpoint saved: Global Step {global_step} with Valid Loss {valid_loss:.6f}", flush=True)
...
def batch_predict(config):
    ...
    # noise_scale: 모델 예측 noise 스케일 보정 파라미터
    noise_scale = config.noise_scale if hasattr(config, 'noise_scale') else 10.0
    ...
    for i in range(n_files):
        ...
        # 역 정규화 및 noise_scale 적용
        pred_trans = pred_out * (global_std + eps) * noise_scale + global_mean
        pred_dct = pred_trans
        
        noise_pred_1d = idct(pred_dct, type=2, norm='ortho')
        noise_pred_list.append(noise_pred_1d)
        
        # 최종 denoised: 원본에서 예측된 노이즈만 제거
...
if __name__ == '__main__':
    ...
    opt.noise_scale = 10.0            # 모델 예측 noise의 스케일 보정 파라미터
    main(opt)
    plt.show()
