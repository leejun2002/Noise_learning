...
# -------------------------------
# arcsinh 변환 + Z-score 정규화 함수 (글로벌 정규화용)
# -------------------------------
...
# -------------------------------
# pipeline_and_save_noise 함수 (글로벌 정규화 + Subplot 시각화)
# -------------------------------
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    7) 전체 noise_data에 대해 arcsinh 변환을 적용한 후, global_mean과 global_std를 계산하여 글로벌 정규화 적용 → norm_data
    ...
    # --- Step 7: Global arcsinh 변환 + 글로벌 Z-score 정규화 ---
    ...
    # --- Step 8: Subplot으로 시각화 (한 Figure 내) ---
...
def batch_predict(config):
    ...
    3) 각 스펙트럼(행)에 대해:
         ...
         (c) 행 단위 DCT 적용
         (d) arcsinh 변환을 적용한 후, 학습에서 저장한 global_mean, global_std를 사용하여 글로벌 정규화 수행
    ...
    # 글로벌 정규화 파라미터는 학습 단계에서 구한 값을 사용
    norm_means = []
    norm_stds = []
    ...
    for i in range(n_files):
        ...
        # (d) arcsinh 변환 후 글로벌 정규화 (학습에서 구한 global_mean, global_std 사용)
