...
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    if config.visualize_steps:
        ...
        plt.colorbar()
        plt.show()
    # 여기서 dct 및 정규화 과정을 생략하고,
    # data_bg_removed (즉, baseline과 BG가 제거된 noise)를 그대로 저장합니다.
    ...
    for epoch in range(config.max_epoch):
        ...
        for idx, noise in enumerate(train_loader):
            ...
            for i in range(spectra_num):
                # DCT 변환: noisy_spectrum의 dct 계수를 입력으로, noise의 dct 계수를 타깃으로 학습
...
def batch_predict(config):
    ...
    #     return
    # # 이 부분은 정규화/역정규화를 사용하지 않으므로 출력된 값은 참고용입니다.
    ...
    for i in range(n_files):
        ...
        # DCT 적용은 그대로 수행 (정규화는 생략)
        resid_dct = dct(data_bg_removed, type=2, norm='ortho')
        
        # 정규화 및 역정규화 없이, 바로 모델 입력
        inp_t = resid_dct.reshape(1, 1, -1)
        ...
        # IDCT 적용 (역정규화 생략)
        noise_pred_1d = idct(pred_out, type=2, norm='ortho')
        noise_pred_list.append(noise_pred_1d)
        
        # 최종 denoised: 원본에서 예측된 noise만 제거
...
if __name__ == '__main__':
    ...
    opt.noise_scale = 10.0            # (사용하지 않으므로 무시)
    main(opt)
    plt.show()
