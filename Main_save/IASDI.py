...
# -------------------------------
# pipeline_and_save_noise 함수 (instance normalization + Subplot 시각화)
# -------------------------------
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    7) 각 행(스펙트럼)에 대해 instance normalization (정규화) 적용 → norm_data (최종 noise 데이터)
    8) norm_data와 각 인스턴스의 mean, std 배열을 .mat 파일에 저장 (키: 'noise', 'train_means', 'train_stds')
    9) 각 단계의 결과를 하나의 Figure 내 Subplot으로 시각화
    """
    eps = 1e-6
    ...
    if not spectra_list:
        ...
        return None
    all_spectra = np.array(spectra_list).T  # [row_new_length, num_files]
    ...
    # --- Subplot 시각화 (한 Figure에 모두 표시) ---
    fig, axs = plt.subplots(3, 3, figsize=(15, 12))
    axs = axs.flatten()

    # For Plotting, we want to plot a few sample spectra with proper x-axis.
    x_axis = np.arange(all_spectra.shape[0])  # 0 ~ row_new_length-1
    ...
    # For DCT, the x-axis should correspond to frequency index (0 ~ col_new_length-1)
    ...
    # For Transpose, now noise_data has shape [col_new_length, row_new_length] -> x-axis: spectral point index
    ...
    # For Instance normalization, norm_data has same shape as noise_data
    ...
    axs[7].set_title("After Instance Norm")
...
# ============================================================================
# [Batch Predict 단계]
# (ALS baseline correction → SVD BG 제거 → DCT → Instance normalization (인스턴스 정규화) →
#  모델 예측 → Inverse Instance normalization (역 인스턴스 정규화) → IDCT →
...
def batch_predict(config):
    ...
    3) 각 스펙트럼(행)에 대해:
         ...
         (c) 행 단위 DCT 적용
         (d) 각 샘플별로 instance normalization 적용 → (norm_val, instance mean, instance std)
         (e) 모델 예측 → 예측된 노이즈 DCT 계수 획득
         (f) Inverse Instance Normalization: 인스턴스 정규화 파라미터를 사용하여 역정규화 후, np.sinh 적용
         (g) IDCT를 통해 노이즈 복원
         (h) 원본 스펙트럼에서 예측된 노이즈 제거 후, ALS baseline과 SVD BG 재결합
    4) 결과를 .mat 파일로 저장하고, 각 샘플의 정규화 파라미터 요약 통계 출력
    """
    import torch
    import numpy as np
    import scipy.io as sio
    ...
    for i in range(n_files):
        ...
        # (d) 각 샘플별 instance normalization 적용
        norm_val, sample_mean, sample_std = instance_normalize(resid_dct, eps=eps)
        norm_means.append(sample_mean)
        norm_stds.append(sample_std)
    ...
    norm_means_arr = np.array(norm_means)
    norm_stds_arr = np.array(norm_stds)
    print("Normalization statistics for batch predict dataset (instance normalization):")
    print("Mean: min = {:.4f}, max = {:.4f}, avg = {:.4f}".format(
        np.min(norm_means_arr), np.max(norm_means_arr), np.mean(norm_means_arr)))
    print("Std:  min = {:.4f}, max = {:.4f}, avg = {:.4f}".format(
        np.min(norm_stds_arr), np.max(norm_stds_arr), np.mean(norm_stds_arr)))
