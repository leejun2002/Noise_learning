...
# 추가: Wavelet baseline 제거 함수
import pywt
def wavelet_baseline(y, wavelet=None, level=None):
    """
    Wavelet decomposition을 사용하여 baseline을 추정하고, 
...
def merge_txt_to_single_key_mat_1280(base_dir, out_mat, row_points=1600, target_files=1280):
    ...
    개수를 target_files로 세로축 보간하여 data_matrix를 .mat 파일로 저장.
    """
...
def train(config):
    ...
    Train 데이터에만 Wavelet baseline 제거 (예상 baseline 확인용) ->
    ...
    # Train 데이터에 대해 Wavelet baseline 제거 적용
    baselines = []
    originals = []
    ...
    print("Wavelet baseline correction applied on train data.")
    # 시각화: 첫 번째 스펙트럼에 대해 원본, baseline, 그리고 baseline 제거된 결과를 확인
    plt.figure("Train: Wavelet Baseline Correction")
    i = 0
    plt.plot(originals[i], label="Original")
    plt.plot(baselines[i], label="Estimated Baseline")
    plt.plot(all_spectra[i, :], label="Baseline Corrected")
    plt.title("Wavelet Baseline Correction on Train Data (Sample)")
    plt.xlabel("Index")
...
def batch_predict(config):
    ...
    도메인 적응(Domain Adaptation) 기법을 사용하여,
    테스트 데이터 전체의 DCT 통계를 계산한 후, 이를 정규화 파라미터로 무조건 업데이트합니다.
    (train 과정에는 baseline 제거가 적용되었으나, test 데이터는 원본 그대로 사용)
    """
    import os
    import sys
    import glob
