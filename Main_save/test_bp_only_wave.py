...
def batch_predict(config):
    ...
    # Wavelet baseline correction (여기서는 그대로 수행)
    ...
    # SVD background removal 생략: processed_matrix를 그대로 사용
    data_for_prediction = processed_matrix  # SVD BG removal을 적용하지 않음
    ...
    # Prediction: For each spectrum, predict the noise using processed_matrix directly
    predicted_noise = np.zeros_like(data_for_prediction)
    for i in range(N):
        row_spec = data_for_prediction[i, :]
    ...
    sample_idx = 0
    # processed_matrix를 이용한 예측 단계
    row_spec = data_for_prediction[sample_idx, :]
    row_dct = dct(row_spec, type=2, norm='ortho')
    row_norm = (row_dct - global_mean) / (global_std + EPS)
    inp_t = torch.from_numpy(row_norm.reshape(1, 1, -1).astype(np.float32))
    if torch.cuda.is_available():
