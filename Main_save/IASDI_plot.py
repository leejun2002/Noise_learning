...
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    7) 각 행에 대해 Instance Normalization 적용 → instance_norm (최종 noise 데이터)
    ...
    print("After instance normalization:", instance_norm.shape)
    plt.figure()
...
# -------------------------------
# Batch Predict 함수
# (Merge → Load → 각 스펙트럼별: ALS Baseline Correction + SVD Background 제거 → DCT → Instance Normalization →
...
def batch_predict(config):
    ...
    3) 각 스펙트럼(행)에 대해:
         ...
         (f) Inverse Instance Normalization 적용 (예측 결과를 해당 인스턴스 스케일로 복원)
         (g) IDCT를 통해 노이즈 복원
    ...
    p = 0.01
    niter = 10
    degree = 3  # (ALS 대신 다항식을 사용하던 경우의 파라미터는 이제 사용하지 않습니다)
