...
###############################################################################
def train(config):
    # Step 1: Process training noise files
    ...
    for epoch in range(config.max_epoch):
        ...
        for idx, noise in enumerate(train_loader):
            noise = noise.squeeze().numpy()  # shape=(batch, 1600)
        ...
        print(f"[Epoch {epoch}] Global Step: {global_step} | "
              f"Train Loss: {epoch_train_loss:.4f} | Valid Loss: {valid_loss:.4f}", flush=True)
