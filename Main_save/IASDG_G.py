...
def train(config):
    ...
    for epoch in range(config.max_epoch):
        ...
        # valid_loss 기준으로 베스트 모델 저장 (valid_loss 기준 상위 3개만 유지)
        if len(best_models) < 3 or valid_loss < max(best_models, key=lambda x: x[0])[0]:
            ...
            print(f"[Save] Checkpoint saved: global_step {global_step} with valid loss {valid_loss:.6f}", flush=True)
...
def batch_predict(config):
    ...
    # noise_scale 파라미터: 모델 출력값에 곱해서 noise 스케일을 보정 (실험적으로 조정)
    ...
    for i in range(n_files):
        ...
        # 역 정규화 시 noise_scale를 곱하여 스케일 보정
...
if __name__ == '__main__':
    opt = DefaultConfig()
    # noise_scale: 모델 예측 noise의 크기를 보정하는 스케일 팩터 (실험적으로 조정)
    opt.noise_scale = 10.0  
