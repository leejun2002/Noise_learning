...
def als_baseline(y, lam=1e5, p=0.01, niter=10, visualize=False):
    ...
    Args:
        ...
        niter (int): 반복 횟수.
        visualize (bool): 개별 ALS 내부 진행률 출력 여부 (batch_predict에서는 False 사용)
    ...
    for i in range(niter):
        ...
        w = p * (y > Z) + (1 - p) * (y < Z)
        if visualize and ((i+1) % max(1, niter//10) == 0 or i == niter-1):
            progress = ((i+1) / niter) * 100
            sys.stdout.write(f"\rALS progress (current row): {progress:.1f}% complete")
            sys.stdout.flush()
    if visualize:
        print("", flush=True)
    return Z
...
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    for j in range(total_cols):
        baseline = als_baseline(all_spectra_2d[:, j], lam=1e5, p=0.01, niter=10, visualize=False)
...
def batch_predict(config):
    ...
    3) 각 스펙트럼에 대해:
         ...
         (d) 최종 denoised spectrum = row_data - noise_pred
    ...
    for i in range(n_files):
        ...
        # (a) ALS baseline correction (visualize=False)
        ...
        # (h) 최종 denoised: **수정된 부분** → 원본에서 예측 노이즈만 제거
...
if __name__ == '__main__':
    opt = DefaultConfig()
    # 시각화를 활성화하기 위한 플래그
    opt.visualize_als = False         # ALS 내부 진행률은 출력하지 않음
