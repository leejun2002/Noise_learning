...
def als_baseline(y, lam=1e5, p=0.01, niter=10):
    ...
    (각 개별 ALS 작업 내부에서는 진행률을 출력하지 않고, 
     전체 ALS 작업 진행률은 파이프라인에서 계산하여 출력합니다.)
    """
    L = len(y)
...
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    for j in range(total_cols):
        ...
        # 전체 ALS 작업 진행 상황 출력 (전체 컬럼 대비 진행률)
        if (j+1) % max(1, total_cols//10) == 0 or j == total_cols - 1:
            progress = ((j+1) / total_cols) * 100
            print(f"Overall ALS progress: {progress:.1f}% complete", flush=True)
    ...
    print("Saved noise_data.mat =>", output_mat, flush=True)
    return output_mat
...
def batch_predict(config):
    ...
    print(f"[완료] 최종 결과 저장 -> {out_name}", flush=True)
