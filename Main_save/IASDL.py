...
#         W = np.diag(w)
#         # np.linalg.solve는 (W + D) * Z = w*y 의 Z를 구합니다.
...
def als_baseline(y, lam=1e5, p=0.01, niter=10):
    ...
    # 2차 미분을 위한 희소 행렬 D 생성
    # D는 (L-2 x L) 크기의 행렬로, 각 행은 [1, -2, 1] 형태를 가집니다.
    diagonals = [np.ones(L), -2 * np.ones(L), np.ones(L)]
    offsets = [0, 1, 2]
    D = diags(diagonals, offsets, shape=(L - 2, L))
    
    # D.T @ D는 (L x L) 크기의 희소 행렬로, 2차 미분의 제곱항을 나타냅니다.
    DTD = D.T @ D
    
    w = np.ones(L)
    for i in range(niter):
        # W는 대각행렬로, 희소 행렬로 생성합니다.
        W = diags(w, 0)
        # (W + lam * DTD) * Z = w * y 를 희소 선형 시스템 풀기(spsolve)를 통해 해결합니다.
        Z = spsolve(W + lam * DTD, w * y)
        # 업데이트: y > Z 인 위치와 y < Z 인 위치에 대해 가중치 업데이트
        w = p * (y > Z) + (1 - p) * (y < Z)
    return Z
...
# 부호 보존 로그 변환 함수
# -------------------------------
def signed_log_transform(signal, eps=1e-6):
    """
    부호를 보존하면서 로그 변환을 적용합니다.
    log1p(x)는 log(1+x)를 계산하여, x가 0 근처일 때 안정적인 계산이 가능합니다.
    """
    return np.sign(signal) * np.log1p(np.abs(signal) + eps)


# -------------------------------
# 로그 변환 후 Z-score 정규화 함수
# -------------------------------
def log_zscore_normalize(signal, eps=1e-6):
    """
    입력 신호에 대해 부호 보존 로그 변환을 적용한 후, 평균과 표준편차를 이용하여 Z-score 정규화합니다.
    
    Returns:
        norm_signal: 정규화된 신호
        mean_val: 로그 변환 후의 평균
        std_val: 로그 변환 후의 표준편차
    """
    signal_log = signed_log_transform(signal, eps=eps)
    mean_val = np.mean(signal_log)
    std_val = np.std(signal_log)
    norm_signal = (signal_log - mean_val) / (std_val + eps)
    return norm_signal, mean_val, std_val


# -------------------------------
# pipeline_and_save_noise 함수 (각 단계별 시각화 포함, ALS baseline correction 적용)
# -------------------------------
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    7) 각 행에 대해 **로그 변환 + Z-score 정규화** 적용 → norm_data (최종 noise 데이터)
    8) 결과를 .mat 파일에 저장 (키: 'noise', 'train_means', 'train_stds')
    
    각 단계 후, 첫 3개 샘플에 대해 시각화합니다.
    """
...
# -------------------------------
# Batch Predict 함수
# (Merge → Load → 각 스펙트럼별: ALS Baseline Correction + SVD Background 제거 → DCT → Log-Zscore 정규화 →
...
def batch_predict(config):
    ...
    3) 각 스펙트럼(행)에 대해:
         (a) Baseline Correction: ALS baseline correction 적용, 제거한 baseline 저장
         (b) SVD Background 제거: baseline 제거된 신호에 대해 SVD 적용하여 배경 추출, 제거한 배경 저장
         (c) 행 단위 DCT 적용
         (d) **Log-Zscore 정규화 적용** (각 인스턴스별 정규화)
         (e) 모델 예측 → 예측된 노이즈 DCT 계수
         (f) Inverse Normalization 적용 (예측 결과를 해당 인스턴스 스케일로 복원)
         (g) IDCT를 통해 노이즈 복원
         (h) 최종 denoised 신호 생성: 원본 스펙트럼에서 예측된 노이즈 제거 후, (a)에서 제거한 baseline과 (b)에서 제거한 배경을 재결합
    4) 결과를 .mat 파일로 저장
    """
    import torch
    import numpy as np
    ...
    for i in range(n_files):
        ...
        # (d) Log-Zscore 정규화 적용
        norm_val, sample_mean, sample_std = log_zscore_normalize(resid_dct, eps=eps)
        ...
        # (f) Inverse Normalization: 원래 스케일 복원
        pred_dct = pred_out * (sample_std + eps) + sample_mean
