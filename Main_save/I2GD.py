...
def block_svd_denoise(matrix, block_size=None, num_sv=None):
    """
    입력 2D 행렬(matrix)을 block_size 단위로 나누어 각 블록에 대해 SVD 수행.
    각 블록에서 상위 num_sv singular value를 0으로 만들어 재구성한 후,
    모든 블록을 합쳐 denoised 결과를 반환.
    """
    M, N = matrix.shape
    denoised = np.zeros_like(matrix)
    for i in range(0, M, block_size[0]):
        for j in range(0, N, block_size[1]):
            block = matrix[i:min(i+block_size[0], M), j:min(j+block_size[1], N)]
            U, s, Vt = svd(block, full_matrices=False)
            s[:num_sv] = 0.0
            block_denoised = U @ np.diag(s) @ Vt
            denoised[i:min(i+block_size[0], M), j:min(j+block_size[1], N)] = block_denoised
    return denoised
...
def train(config):
    """
    Au film(또는 잡음만 있는 데이터) txt -> 행/열 보간 ->
    2D Block SVD로 상위 num_sv singular value를 제거하여 잡음만 추출
    -> global DCT 통계 계산 → 모델 학습
    (wavelet baseline correction은 사용하지 않음)
    """
    ...
    # 2D Block SVD denoising
    block_size = (config.block_height, config.block_width) if hasattr(config, 'block_height') else (50, 50)
    num_sv = config.num_sv if hasattr(config, 'num_sv') else 1
    noise_processed = block_svd_denoise(all_spectra, block_size=block_size, num_sv=num_sv)
    print(f"[train pipeline] Shape after 2D Block SVD noise extraction: {noise_processed.shape}", flush=True)

    # 제거한 배경 계산: 원본에서 SVD 결과를 뺀 값
    removed_background = all_spectra - noise_processed

    # Visualization: 원본, SVD 적용 후, 제거된 배경을 비교
    plt.figure("SVD Comparison on Train Data")
    plt.subplot(1,3,1)
    ...
    plt.subplot(1,3,2)
    plt.plot(noise_processed[0], label="After 2D SVD")
    plt.title("Denoised (2D SVD)")
    plt.legend()

    plt.subplot(1,3,3)
    plt.plot(removed_background[0], label="Removed Background")
    ...
    # Global DCT 통계 계산 (Global Z-score normalization)
...
def batch_predict(config):
    """
    실제 시료(txt) -> row/col 보간 -> DCT → 모델 예측 → 역 DCT → denoised 계산
    도메인 적응 기법을 사용하여,
    테스트 데이터 전체의 DCT 통계를 계산한 후, 이를 정규화 파라미터로 무조건 업데이트합니다.
    (Train 과정에는 wavelet baseline correction 없이 원본 데이터를 사용합니다)
    """
    import os
    import sys
    import glob
...
if __name__ == '__main__':
    opt = DefaultConfig()
    # 예: rank_bg, block_height, block_width, num_sv 등은 config에서 설정
    main(opt)
    plt.show()
