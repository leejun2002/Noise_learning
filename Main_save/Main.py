...
def train(config):
    ...
    for epoch in range(config.max_epoch):
        ...
        for idx_v, noise in enumerate(valid_loader):
            ...
            # 전치
            clean_spectra = clean_spectra.T
            noisy_spectr= clean_spectra + noise
            # 입출력 정의a 
            input_coef, output_coef = np.zeros(np.shape(noisy_spectra)), np.zeros(np.shape(noise))
            # 전처리 수행, DCT 변환
            for index in range(spectra_num):
...
def predict(config):
    ...
    for file in filenames:
        if os.path.splitext(file)[1] == '.txt':
            ...
            new_name = config.predict_save_root + '/dn_' + file
            # 테스트 데이터 로드
            tmp = np.loadtxt(name, delimiter='\t').astype(float)
            wave, x = tmp[:, 0], tmp[:, 1]
            # 데이터 전처리: 슬라이딩 윈도우 필터링 + DCT 변환
            ...
            if torch.cuda.is_available():
                inpt = inpt.cuda()
            yt = model(inpt).detach().cpu().numpy() # *예측*
            yt = yt.reshape(-1, )
            # IDCT 변환
            ...
            # i = i + 1
            # plt.subplot(3, 3, i)
            plt.plot(wave, x, label='Noisy Spectrum')
            plt.plot(wave, Y, label='Denoised Spectrum')
