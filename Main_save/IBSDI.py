...
# -------------------------------
# Train 전처리 함수
# (Interpolation → Baseline Correction → SVD 배경 제거 → DCT → Transpose → Instance Normalization → .mat 저장)
# -------------------------------
def pipeline_and_save_noise(
    ...
    """
    이 함수는 다음 단계를 거칩니다.
      1) 각 txt 파일에서 데이터를 로드하고, row 방향 cubic 보간 수행 → all_spectra (shape: row_new_length x num_files)
      2) 열 방향 보간 → all_spectra_2d (shape: row_new_length x col_new_length)
      3) 각 열에 대해 3차 다항식 baseline correction 수행 → data_bc_removed
      4) baseline 제거된 데이터에 대해 SVD를 적용하여 배경 제거 → data_bg_removed
      5) 각 열에 대해 DCT 적용 → dct_result
      6) 행렬 전치 → noise_data (shape: col_new_length x row_new_length)
      7) 각 행(스펙트럼)에 대해 Instance Normalization 적용 → 최종 noise 데이터
      8) 결과를 .mat 파일로 저장 (키: 'noise', 'train_medians', 'train_iqrs')
    
    각 단계 후, 첫 3개 스펙트럼의 결과를 시각화합니다.
    """
    ...
    # 시각화: 첫 3개 열(스펙트럼) → 주로 열 방향 보간은 각 행의 변화이므로, 여기서는 행 기준 플롯
    plt.figure()
    ...
    for j in range(all_spectra_2d.shape[1]):
        x = np.arange(all_spectra_2d.shape[0])
        degree = 3
        p = np.polyfit(x, all_spectra_2d[:, j], degree)
        baseline = np.polyval(p, x)
        data_baseline[:, j] = baseline
    data_bc_removed = all_spectra_2d - data_baseline
    print("After baseline correction:", data_bc_removed.shape)
    # 시각화: 첫 3개 스펙트럼 baseline와 제거 결과
    plt.figure()
    ...
    # 시각화: 첫 3개 스펙트럼 정규화 결과
    plt.figure()
    for idx in range(min(3, instance_norm.shape[0])):
        plt.plot(instance_norm[idx, :], label=f"Sample {idx+1}")
    plt.title("After Instance Normalization")
    plt.xlabel("Spectral Point")
...
# -------------------------------
# Train 함수: 노이즈 전처리 후 데이터셋 구성 및 U-Net 학습 진행
# -------------------------------
def train(config):
    """
...
def batch_predict(config):
    ...
    3) 각 스펙트럼(행)에 대해:
         (a) Baseline Correction: 3차 다항식 피팅으로 baseline 추정 및 제거, 제거한 baseline 저장
         ...
         (g) IDCT를 통해 노이즈 복원
         (h) 최종 denoised 신호 생성: 원본 스펙트럼에서 예측된 노이즈를 제거한 후, baseline과 SVD에서 제거한 배경을 재결합
    4) 결과를 .mat 파일로 저장
    """
    import torch
    import numpy as np
    ...
    baseline_list = []     # Baseline Correction에서 제거한 baseline
    noise_pred_list = []
    denoise_list = []
    eps = 1e-6
    degree = 3  # 3차 다항식
    ...
    for i in range(n_files):
        ...
        # (a) Baseline Correction: 3차 다항식 피팅
        p = np.polyfit(x, row_data, degree)
        baseline = np.polyval(p, x)
        ...
        # (h) 최종 denoised 신호 생성: 원본 스펙트럼에서 예측된 노이즈 제거 후, baseline과 SVD에서 제거한 배경을 재결합
