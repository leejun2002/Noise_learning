...
def instance_normalize(signal, eps=1e-6):
    ...
    Args:
        signal (1D np.array): 정규화할 신호.
        eps (float): 0으로 나누는 것을 방지하기 위한 작은 값.
        
    Returns:
        norm_signal (1D np.array): 정규화된 신호.
        median_val (float): 신호의 중앙값.
        iqr_val (float): 신호의 IQR.
    """
...
# -------------------------------
# 1) Noise 전처리 (Interpolation → SVD(BG 제거) → DCT → Transpose → wavelet shrinkage → Instance Normalization → .mat 저장)
# -------------------------------
def pipeline_and_save_noise(
    ...
    7) 각 행에 대해 wavelet shrinkage 적용 후, instance normalization 수행
    ...
    # 7) Instance Normalization (각 행별)
    eps = 1e-6
    instance_norm = np.zeros_like(wavelet_norm)
    instance_medians = []
    instance_iqrs = []
    for i in range(wavelet_norm.shape[0]):
        norm_row, med_val, iqr_val = instance_normalize(wavelet_norm[i, :], eps=eps)
        instance_norm[i, :] = norm_row
        instance_medians.append(med_val)
        instance_iqrs.append(iqr_val)
    print("Instance normalization applied.")

    # 8) 결과 저장 (.mat 파일에 instance normalized noise 및 각 인스턴스 통계 저장)
...
def merge_txt_to_single_key_mat_1280(
    ...
    out_mat="/home/jyounglee/NL_real/data/batch_predict/merged_spectra_ISDWI.mat",
    row_points=1600,
    target_files=1280
):
    """
...
def batch_predict(config):
    ...
    # (1) txt 파일 통합 → .mat 생성
    merged_mat = os.path.join(config.batch_predict_root, "merged_spectra_isdwi.mat")
    ...
    for i in range(n_files):
        ...
        # (d) Instance Normalization 적용 (각 샘플 별로)
        norm_val, sample_median, sample_iqr = instance_normalize(resid_dct_ws, eps=eps)
        ...
        # (h) 최종 denoise: 원본 스펙트럼에서 예측된 노이즈 제거
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    out_name = os.path.join(out_dir, "ISDWI.mat")
