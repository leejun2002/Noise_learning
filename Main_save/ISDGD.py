...
def train(config):
    ...
    SVD로 가장 큰 rank_bg 성분 제거 → 남은 것을 노이즈로 간주
    ...
    # SVD: 가장 큰 rank_bg개의 특이값(배경)을 제거하여 순수 잡음만 남김
...
def batch_predict(config):
    ...
    이때, 도메인 적응(Domain Adaptation) 기법을 적용하여,
    만약 Test 데이터의 DCT 통계가 학습 시 global_mean/std와 크게 다르면,
    이를 반영하도록 정규화 파라미터를 조정합니다.
    """
    import os
    import sys
    import glob
    ...
    # 학습 시 저장한 global_mean, global_std 불러오기
    ...
    print(f"[batch_predict] Loaded train global_mean={global_mean}, global_std={global_std}")

    # Domain Adaptation: 테스트 데이터의 일부 스펙트럼 DCT 통계를 계산하여,
    # 만약 학습 시 global_mean/std와 10% 이상 차이가 나면 이를 업데이트합니다.
    N, L = raw_data_matrix.shape
    sample_N = min(100, N)
    test_dcts = []
    for i in range(sample_N):
        test_dcts.append(dct(raw_data_matrix[i, :], type=2, norm='ortho'))
    test_dcts = np.array(test_dcts)
    test_mean = np.mean(test_dcts)
    test_std = np.std(test_dcts)
    domain_threshold = getattr(config, "domain_threshold", 0.1)  # 기본 10%
    if abs(test_mean - global_mean) / abs(global_mean) > domain_threshold or \
       abs(test_std - global_std) / abs(global_std) > domain_threshold:
        print(f"[Domain Adaptation] Adjusting normalization parameters:")
        print(f"  Train: mean={global_mean:.4f}, std={global_std:.4f}")
        print(f"  Test:  mean={test_mean:.4f}, std={test_std:.4f}")
        # 간단히 테스트 통계로 업데이트 (또는 가중평균 사용 가능)
        global_mean = test_mean
        global_std = test_std
...
if __name__ == '__main__':
    ...
    # opt.rank_bg = 1
    # 도메인 적응 임계값 (예: 10% 차이가 나면 업데이트)
    if not hasattr(opt, "domain_threshold"):
