...
def pipeline_and_save_noise(base_folder, output_mat, config, subfolders=("1", "2", "3"),
    ...
    """
    1) base_folder 내의 txt 파일들을 읽어 row, col 보간 수행 → all_spectra, all_spectra_2d  
...
def batch_predict(config):
    ...
    3) 각 스펙트럼에 대해:
         ...
         (f) **역 전역 정규화**: 예측값에 global_std, global_mean를 적용한 후 np.sinh  
         (g) IDCT 적용하여 노이즈 복원  
         (h) 원본 스펙트럼에서 예측 노이즈 제거 후 baseline, BG 재결합하여 denoised spectrum 생성  
    4) 결과를 .mat 파일에 저장
    """
    import torch
    import numpy as np
    ...
    # config.train_data_root 내 noise_data.mat에 저장되어 있다고 가정합니다.
    ...
    if 'global_mean' not in train_mat or 'global_std' not in train_mat:
        ...
        return
    # 만약 스칼라라면 item() 사용
    # global_mean = train_mat['global_mean'].item() if np.ndim(train_mat['global_mean']) == 0 else train_mat['global_mean']
    # global_std = train_mat['global_std'].item() if np.ndim(train_mat['global_std']) == 0 else train_mat['global_std']
    ...
    for i in range(n_files):
        ...
        # (d) **Global normalization**: 학습 시 구한 global_mean, global_std 사용
        ...
        # (f) **역 Global 정규화**: 예측값에 global_std, global_mean 적용 후 np.sinh
